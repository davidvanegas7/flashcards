...
<x-app-layout>
    ...
    >
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                <h2 class="text-2xl font-bold text-gray-800">{{ __('Edit Card')}}</h2>
                <p class="mt-1 text-sm text-gray-600">
                    {{ __('Editing card from deck:') }} <span class="font-medium">{{ $card->deck->name }}</span>
                </p>
            </div>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <form method="POST" action="{{ route('cards.update', ['deck' => $card->deck, 'card' => $card]) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="flex items-center justify-end gap-4">
                                <x-secondary-button type="button" onclick="window.location='{{ route('decks.show', $card->deck) }}'">
                                    {{ __('Cancel') }}
                                </x-secondary-button>
